...
class AuthenticationTestIT extends BankModuleApplicationTests{
    ...
    def "test authentication of #endpoint"() {
        ...
        when:
        ResponseEntity<String> response = restTemplate.withBasicAuth(user.username, user.passwordHash)
                .getForEntity(new URI("http://localhost:8080/home-operator/${endpoint}"),String.class)
        then:
